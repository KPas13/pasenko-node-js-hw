...
let logServerPort = process.env.SERVER_PORT || (data && data.network && data.network.port) || 8000;
let logHostname = process.env.HOST_NAME || (data && data.network && data.network.hostname) || 'localhost';
...
const formattedLogServerPort = formatToUpperCase(logServerPort);
const formattedLogHostname = formatToUpperCase(logHostname);

export { formattedLogLevel as logLevel, formattedLogAppender as logAppender, formattedLogFormat as logFormat, formattedLogDelimetter as logDelimiter, formattedLogServerPort as logServerPort, formattedLogHostname as logHostname};
