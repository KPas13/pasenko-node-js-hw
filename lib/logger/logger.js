...
import * as formatStrategy from './formatStrategy.js';
...
const format = formatStrategy.getFormat();

function executeLog(level, category, message) {
    if (scoreLevel[level] <= config.scoreLevel) {
        for (const k of appender) {
            k.log(
                format.formatMessage(Date.now(), level, category, message),
            );
        }
    }
}
